...
from utils.generator import set_generator
...
def set_generator_classifier(root_dir, preprocessing_function=None, batch_size=16, number_of_images=100):
    ...
    generator = DataGenerator(input_data, batch_size=batch_size, shuffle_per_label=True, shuffle_all_dataset=False,
                              preprocessing_function=preprocessing_function, num_iter=number_of_images)
...
def set_generator_object_detector(image_dir, bb_dir, preprocessing_function=None, batch_size=16,
    ...
    for filename in os.listdir(image_dir):
        ...
        bb_filename = os.path.join(bb_dir, filename)
        input_data.append((bb_filename, image_filename))
