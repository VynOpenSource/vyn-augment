...
from ai_data_processing.image import skew, checker, swap_patches, check_range, create_circular_mask, convert_to_absolute, create_grid_masks
...
class Augmentor(object):
	...
	def __init__(self, operations, std_noise=50, no_repetition=False, synthetic_image_creator=None, input_synthesizer_size=None, seed=None):
		'''
		...
		# self.rescale = lambda im: (255 * (im.astype(np.float) - np.min(im)) / (np.max(im) - np.min(im))).astype(np.uint8)
			...
			if type_data == 'numpy':
				...
				for i, image in enumerate(output):
					...
					if len(image.shape) == 2 and channel == 1:
						image = image[..., None]
					elif (len(image.shape) == 2 and channel == 3) or (len(image.shape) == 3 and image.shape[2] == 1 and channel == 3):
	...
	def apply_on_objects(self, images, values, **kwargs):
		...
		if not isinstance(mask_matching, dict):
			raise ValueError(f'In {op}_object, Mask positions must be a dictionary with keys as images and values a list of masks positions')
		...
		if op.lower() == 'sample_pairing':
			...
			for i, bbs_i in enumerate(bbs):
				if len(labels[i]) != len(bbs_i):
					raise ValueError('In sample pairing between images, the number of labels must be the same as the number of bounding boxes per image')
			...
			if len(mix_images) != len(mix_labels):
				raise ValueError('In sample pairing between images, the number of labels must be the same as images (mix_images and mix_labels).')
		...
		if not len(images) == len(bbs):
			raise ValueError(f'In {op}_object, the bbs must be a list with as many elements as images without counting masks')

		if prob < 0 or prob > 1:
			raise ValueError(f'In {op}_object, the probability of flipping a bounding box must be greater than 0 and smaller than 1')
		...
		fun = getattr(self, op)
		imresize2 = lambda image, nh, nw: imresize(image, (nh, nw), preserve_range=True, mode='constant', anti_aliasing=True)
		for i in positions:
			masks = mask_matching.get(i, [])
			for ii, bb in enumerate(bbs[i]):
				inputs = []
				if len(bb) != 4:
					raise ValueError(f'In {op}_object, bounding boxes found with less than 4 elements. They must be (x0, y0, x1, y1)')
				if np.random.rand() < prob:
					if op in self.numpy_fun:
						...
						if op.lower() == 'sample_pairing':
							...
							kwargs['mix_images'] = [imresize(mix_images[p], inputs[0].shape[:2], anti_aliasing=True, preserve_range=True)]
	...
	def blur(self, images, values, **kwargs):
		...
		if not hasattr(values, '__len__') or len(values) != 3:
			raise ValueError('In blur, the number of elements must be 3. The type of blurrines and the minimum and maximum value of the parameter to change')
		...
		for i, image in enumerate(images):
			if no_mask_positions[i]:
				...
				if kernel == 'gaussian':
					factor = int(np.round(checker('blur', 'standard deviation of Gaussian kernel', values, 2, 1e-6, min(*image.shape[:2]))))
					im = gaussian(image, factor, preserve_range=True, multichannel=True, **filter_parameters)
					output.append(self.rescale(im))
				elif kernel == 'median':
					factor = int(np.round(checker('blur', ' Size of the median kernel', values, 2, 1, min(*image.shape[:2])/4)))
					selem = np.ones((factor, factor))
					im = median(image, selem[:,:,None], **filter_parameters)
					output.append(self.rescale(im))
				else:
					raise ValueError(f'Kernel {kernel} not found. Currently, only Gaussian, median and random are available')
			else:
				output.append(image)
	...
	def change(self, images, values, bbs, **kwargs):
		...
		if not isinstance(type_change, str):
			raise TypeError(('The first input of values must be a string with values' + '{}, ' * len(self.locations)).format(
					*self.locations)[:-2])
		if type_change.lower() not in self.locations:
			raise ValueError((f'In change_object, the value {type_change} does not exists it should be one of ' + '{}, ' * len(
		...
		if not isinstance(mix_images, list) or (type_change == 'inter' and len(mix_images) == 0):
			raise ValueError('In change_objects, the mix_image parameter is mandatory and it must be a list with images')

		number_copies = kwargs.get('number_copies', 1)
		if not isinstance(mix_images, (int, list, tuple)) or len(number_copies) != 2:
			raise ValueError('In change_objects, the number_copies parameter must be an integer, list or tuple with 2 values at most')
		...
		for i, image in enumerate(images):
			...
			for j, bb in enumerate(bbs[i]):
				if len(bb) != 4:
					raise ValueError(f'In change_object, bounding boxes found with less than 4 elements. They must be (x0, y0, x1, y1)')
	...
	def crop(self, images, values, **kwargs):
        ...
        :param kwargs: For this operation, the only extra parameter is the whether an image is a mask.
        				...
        				- keep_size (optional): Boolean Keep the same size as the cropping withot resizing it to the original size
	...
	def flip(self, images, values, **kwargs):
		...
		if values.lower() == 'horizontal' or values.lower() == 'hor' or values.lower() == 'both' or values.lower() == 'all':
			...
			if bbs:
				for i, image in enumerate(images):
					bbs_out.append([[image.size[0] - bbi[2], bbi[1], image.size[0] - bbi[0], bbi[3]] for ii, bbi in enumerate(bbs[i])])
				bbs = bbs_out
		...
		if values.lower() == 'vertical' or values.lower() == 'ver' or values.lower() == 'both' or values.lower() == 'all':
			...
			if bbs:
				for i, image in enumerate(images):
					bbs_out.append([[bbi[0], image.size[1] - bbi[3], bbi[2], image.size[1] - bbi[1]] for ii, bbi in enumerate(bbs[i])])
				bbs = bbs_out
	...
	def grid_mask(self, images, values, **kwargs):
		...
		if not hasattr(values, '__len__') or len(values) != 12:
			raise ValueError('The number of values for the grid_mask operation must be a list or tuple with 12 values. The range of the initial point, square size and distance between square in x and y for the three of them')
		...
		params = []
		name = 'grid_mask'
		name_params = ['initial x position', 'initial y position', 'width square', 'height square', 'x distance between squares', 'y distance between squares']
		for i in range(len(values) // 2):
	...
	def rotate(self, images, values, **kwargs):
		...
		for i, image in enumerate(images):
			...
			if bbs:
				...
				rotate_point = lambda point: np.round(np.dot(rot_mat.transpose(), (point - center)) + center).astype(int)
				...
				for ii, bbi in enumerate(bbs[i]):
					...
					bbs[i][ii] = self._correct_bb([bbs[i][ii][iii] * factor[iii % 2] for iii in range(len(bbi))], [w, h])
			# image = Image.fromarray(self.rescale(image))
	...
	def sample_pairing(self, images, values, **kwargs):
		...
		if not self.check_groups_images_equal_size(images, mix_images):
			print('For {}, the size of all the images, including the ones for mixing must be the same. Aborting'.format(name))
			return images
	...
	def synthesise_image(self, images, values):
		"""
		Use an already trained moel to synthesize new data
		"""
		self.synthetic_image_creator = synthetic_image_creator
		self.input_synthesizer_size = input_synthesizer_size
	...
	def translate(self, images, values, **kwargs):
		...
		if not len(use_colour) == 1 and not len(use_colour) == 3:
			raise ValueError('In translate: use_colour must be a single number or a list, tuple or array with 1 or 3 values')
	...
	def zoom(self, images, values, **kwargs):
		...
		if bbs:
			if dif_h != 0 or dif_w != 0:
				for i, bbs_i in enumerate(bbs):
					for ii, bbi in enumerate(bbs_i):
						bbs[i][ii] = self._correct_bb([bbi[iii] * factor + diff[iii%2] for iii in range(len(bbi))], [h, w])
...
def normal_image():
	...
	d = (500, 343)
	#im1 = imread('/home/isaac/containers/Data/weather/training/snow/1609425.jpg')
	#im2 = imread('/home/isaac/containers/Data/weather/training/sunny/6777099.jpg')
	root = os.path.realpath('../../../tests/augmentor')
	im1 = imread(os.path.join(root, 'car2.png'))
	im2 = imread(os.path.join(root, 'mug1.jpg'))
	imresize2 = lambda x: imresize(x, d, anti_aliasing=True, preserve_range=True)
	ims = [imresize2(im1), imresize2(im2)]
	labels = [[0, 1], [1, 0]]

	# config = {'blur': {'values': ('gaussian', 0.7, 1.0), 'prob': 1.0 }}
	# config = {'brightness': (0.8, 1.2)}
	# config = {'colour_balance': (0.2, 3)}
	# config = {'contrast': (0.5, 1.0)}
	# config = {'crop': (0.4, 0.9)}
	# config = {'flip': {'values': ['random'], 'probability': 1}}
	# config = {'greyscale': {'probability': 1.0}}
	# config = {'grid_mask': (0,0.1, 0,0.1, 0.05,0.15, 0.1,0.15, 0.1,0.15, 0.1,0.2)}
	# config = {'illumination': ['blob_negative',0.1,0.2, 100,150]}
	# config = {'noise': {'values': (5, 16), 'use_gray_noise':True}}
	# config = {'occlusion': {'values':('hide_and_seek', 0.2,0.5,0.1,0.3), 'num_patches':3}}
	config = {'posterisation': (5, 10)}
	# config = {'rotate': {'values':(20, 60), 'use_colour':255}}
	# config = {'rgb_swapping': []}
	# config = {'sample_pairing': {'values': (0.6,0.9, ims, labels), 'probability':1.0}}
	# config = {'sharpness': (0,0.3)}
	# config = {'shear': ('RANDOM', 10, 15)}
	# config = {'skew': ('RANDOM', 0.3, 0.6)}
	# config = {'solarise': {'probability': 1.0}}
	# config = {'translate': {'values': ('all', 0.1, 0.3, 0.5, 0.7), 'probability': 1.0, 'use_colour':[120,120,120], 'use_replication':True}}
	# config = {'zoom': {'values':(0.2, 0.3), 'use_colour':-1, 'use_replication':False}}

	# im1 = imread('/home/isaac/containers/Augmentation_on_the_fly/images/squirrel_original.jpg')
	# aug = Augmentor(config)
	# im = aug.run([im1])
	# imsave('/home/isaac/containers/Augmentation_on_the_fly/images/squirrel_aug2.jpg', im[0])

	image = os.path.join(root, 'car1.png')
	# image = '/home/isaac/containers/Data/weather/training/rain/249959284.jpg'
	# image = '/home/isaac/Presentations/Nvidia-poster/python-image/image.png'
	aug = Augmentor(config)
	label = [0, 1]
	im = aug.run([imread(image)], labels=[label], mask_positions=[])  # , mask_positions=[0]
...
def test_object():
	...
	aug = Augmentor(config)
	label = [0, 1]
	im = aug.run([imread(image)], labels=[label], object_label=[[label]], mask_positions=[], bounding_boxes=bb_im)  # , mask_positions=[0]
...
def test():
	...
	image = os.path.join(root, 'mug1.jpg')
	image = imread(image)
	mix_images = [imresize(image, image.shape,anti_aliasing=True, preserve_range=True).astype(np.uint8) for image in mix_images]
